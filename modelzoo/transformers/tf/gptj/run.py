...
from modelzoo.transformers.tf.gptj.data import eval_input_fn, train_input_fn
from modelzoo.transformers.tf.gptj.model import model_fn
from modelzoo.transformers.tf.gptj.utils import (
...
def validate_params(params, cs1_modes, output_layer_name=None):
    ...
    if (
        ...
        assert runconfig_params["mode"] in cs1_modes, (
            "To run this model on the Cerebras System, please use one of the"
            + " following modes: ,".join(cs1_modes)
        )

    # check warm start settings
    if output_layer_name is not None and "bias" in output_layer_name:
        tf.compat.v1.logging.warning(
            "Bias is being disabled for FFN layers as well via current"
            + " settings. Please ensure this is desired behavior!!"
        )
...
def run(
    ...
    :params list cs1_modes: Modes for enabling runs on cs1. Currently, only
        training should be supported for weight streaming mode.
    """
    # update and validate runtime params
    runconfig_params = params["runconfig"]
    update_params_from_args(args, runconfig_params)
    validate_params(params, cs1_modes, output_layer_name)
    params = update_input_checkpoint_steps(params)
    ...
    # use this to load pre-trained checkpoints
