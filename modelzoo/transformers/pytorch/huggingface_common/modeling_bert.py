...
from modelzoo.transformers.pytorch.huggingface_common.configuration_bert import (
    BertConfig,
)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        query_layer = self.transpose_for_scores(
            ...
            1 / math.sqrt(self.attention_head_size), dtype=self.dtype
        )
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(
            config.hidden_size,
            config.hidden_size,
            config.use_ffn_bias_in_attention,
        )
        self.LayerNorm = nn.LayerNorm(
            config.hidden_size, eps=config.layer_norm_eps
        )
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(
            config.hidden_size, config.intermediate_size, config.use_ffn_bias
        )

        if isinstance(config.hidden_act, str):
            self.intermediate_act_fn = ACT2FN[config.mlm_nonlinearity]
        else:
            self.intermediate_act_fn = config.mlm_nonlinearity
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(
            config.hidden_size,
            config.hidden_size,
            bias=config.use_ffn_bias_in_mlm,
        )
        if isinstance(config.hidden_act, str):
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        _std = self.config.initializer_range
        _a = _std * -2.0
        _b = _std * 2.0
...
class BertForPreTrainingLoss(nn.Module):
    def __init__(
        self, disable_nsp=False, mlm_loss_weight=1.0, label_smoothing=0.0,
    ):
        super(BertForPreTrainingLoss, self).__init__()
    ...
    def forward(
        ...
        masked_lm_loss *= masked_lm_weights.view(-1)
        masked_lm_loss = torch.sum(masked_lm_loss) / labels.shape[0]

        # Implements the `batch_size` MLM weighting mode, which does mean
        ...
        if mlm_loss_scale is None:
            masked_lm_loss *= self.mlm_loss_weight
        ...
        if not self.disable_nsp:
            ...
            next_sentence_loss = loss_fct(
                ...
                next_sentence_label.view(-1).long(),
            )
            total_loss += next_sentence_loss.half()
...
class BertForPreTraining(BertPreTrainedModel):
    def __init__(self, config, mlm_loss_weight=1.0, label_smoothing=0.0):
        super().__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config)
        self.loss_fn = BertForPreTrainingLoss(
            disable_nsp=False,
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.loss_fn = BertForSequenceClassificationLoss(
            config.num_labels, self.problem_type
        )
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size,2)`, `optional`):
            Labels for position (index) of the [start,end] of the labelled span for computing the token classification loss.
            Positions should be clamped to the length of the sequence (:obj:`sequence_length`). Position outside of the
            sequence should be indicated with a zero label_mask:
        label_weight (:obj:`torch.HalfTensor` of shape :obj:`(batch_size,2)`, `optional`):
            Weight of each (start,end) position in labels. Use 0 for positions
            which should not contribute to the loss (i.e. positions out of
            bounds of the sequence length). The magnitude can be used to convey
            batch-level scaling factors.
        """
        return_dict = (
            return_dict
            if return_dict is not None
        ...
        total_loss = self.loss_fn(logits, labels, label_weights)
