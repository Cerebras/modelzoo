...
class ModuleUtilsMixin:
    ...
    def invert_attention_mask(self, encoder_attention_mask: Tensor) -> Tensor:
        ...
        # encoder_attention_mask.transpose(-1, -2))
        encoder_attention_mask = encoder_attention_mask.to(
            dtype=torch.float16
        )  # fp16 compatibility.

        # Since `encoder_attention_mask` is passed as `1.0` for positions we
        # want to attend and `0.0` for masked positions, this operation will
        # "invert" the mask due to the "negative infinity" scaling at the end.
        ...
        return encoder_attention_mask * -1e4
    ...
    def estimate_tokens(
        ...
        """
        token_inputs = [
            tensor for key, tensor in input_dict.items() if "input" in key
        ]
        if token_inputs:
...
class PreTrainedModel(
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        for output_embedding, input_embedding in zip(
            ...
            if self.config.torchscript:
                output_embedding.weight = nn.Parameter(
                    input_embedding.weight.clone()
                )
            else:
    ...
    def _get_resized_lm_head(
        ...
        if is_deepspeed_zero3_enabled():
            ...
            with deepspeed.zero.GatheredParameters(
                old_lm_head.weight, modifier_rank=0
            ):
                if torch.distributed.get_rank() == 0:
...
def apply_chunking_to_forward(
    ...
    *input_tensors,
    ...
    assert all(
        input_tensor.shape[chunk_dim] == tensor_shape
