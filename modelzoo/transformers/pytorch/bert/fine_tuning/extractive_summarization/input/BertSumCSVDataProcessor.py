...
class BertSumCSVDataProcessor(torch.utils.data.IterableDataset):
    ...
    def create_dataloader(self, is_training=True):
        ...
        dataloader = torch.utils.data.DataLoader(
            ...
            prefetch_factor=self.prefetch_factor if self.num_workers else 2,
    ...
    def __iter__(self):
        ...
        for data_row in self.load_buffer():
            ...
            (input_token_ids, labels, segment_ids, cls_indices,) = tuple(
                map(convert_str_to_int_list, data_row.values())
            )
