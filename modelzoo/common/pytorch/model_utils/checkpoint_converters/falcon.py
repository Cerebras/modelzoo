...
class Converter_Falcon_Attention_HF_CS19(Converter_GPT_Neox_Attention_HF_CS17):
    def __init__(self):
        super().__init__()
        self.rules = [
            ConversionRule(
    ...
    def get_config_converter_class() -> BaseConfigConverter:
        return ConfigConverter_Falcon_HF_CS19


class Converter_Falcon_Headless_HF_CS19(BaseCheckpointConverter_HF_CS):
    def __init__(self):
        super().__init__()
        self.rules = [
            # Embedding:
            ...
            ConversionRule(
                [
                    ...
                    Converter_Falcon_Attention_HF_CS19(),
                ],
                action=None,
            ),
            # mlp
...
class Converter_Falcon_HF_CS19(BaseCheckpointConverter_HF_CS):
    def __init__(self):
        super().__init__()
        self.rules = [
            ConversionRule(
...
class ConfigConverter_Falcon_HF_CS19(BaseConfigConverter_HF_CS):
    def __init__(self):
        super().__init__()
        self.rules = [
            # Embedding
            ...
            # Decoder Block
            ConversionRule(["hidden_size"], action=self.convert_hidden_size,),
            ConversionRule(
    ...
    def pre_config_convert(
        ...
        # Apply defaults
        for key in self.defaults[from_index]:
            if key not in config:
                config[key] = self.defaults[from_index][key]
    ...
    def post_config_convert(
        ...
        # Apply defaults
        for key in self.defaults[1 - from_index]:
            if key not in new_config:
                new_config[key] = self.defaults[1 - from_index][key]
