...
class Converter_LlamaAttention_HF_CS(BaseCheckpointConverter_HF_CS):
    def __init__(self):
        ...
        self.rules = [
            ConversionRule(
                ...
                action=self.convert_with_interleaving,
            ),
            ConversionRule(
                [
    ...
    def convert_with_interleaving(
        ...
        if from_index == 0:
            ...
            tensor = self.reverse_interleave_helper(tensor, cs_config)
...
class ConfigConverter_LLaMa_HF_CS19(BaseConfigConverter_HF_CS):
    ...
    def post_config_convert(
        ...
        # Apply defaults
        for key in defaults[to_index]:
            if key not in new_config:
                new_config[key] = defaults[to_index][key]
