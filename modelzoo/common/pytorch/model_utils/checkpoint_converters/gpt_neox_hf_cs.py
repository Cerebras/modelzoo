...
class Converter_GPT_Neox_Attention_HF_CS17(BaseCheckpointConverter_HF_CS):
    ...
    def reverse_interleave_helper(self, t, cs_config):
...
class Converter_GPT_Neox_Headless_HF_CS17(BaseCheckpointConverter_HF_CS):
    ...
    def pre_model_convert(
        ...
        if from_index == 0:
            logging.warning(
                "CS-1.7 GPT Neox has a language model head (lm_head) "
                "while HF GPTNeoxModel does not. Initializing lm_head to default."
            )
    ...
    @staticmethod
    @converter_notes(
        notes="""HF GPTNeoXForCausalLM <-> CS 1.7 GPTJModel (configured as neox)
...
class Converter_GPT_Neox_Headless_HF_CS18(Converter_GPT_Neox_Headless_HF_CS17):
    ...
    @staticmethod
    @converter_notes(
        notes="""HF GPTNeoXForCausalLM <-> CS 1.8 GPTJModel (configured as neox)
    The HF model doesn't contain a language model head while the CS one does.
    When converting to CS, the exported checkpoint will contain a language model
    head initialized to default random values. When converting to HF, the
    language model head will be dropped.
...
class Converter_GPT_Neox_LMHeadModel_HF_CS17(BaseCheckpointConverter_HF_CS):
    ...
    @staticmethod
    @converter_notes(
        notes="HF GPTNeoXForCausalLM <-> CS 1.7 GPTJModel (configured as neox) with LM head"
    )
    def formats() -> Tuple[str, str]:
        return ("hf", "cs-1.7")
...
class Converter_GPT_Neox_LMHeadModel_HF_CS18(BaseCheckpointConverter_HF_CS):
    ...
    @staticmethod
    @converter_notes(
        notes="HF GPTNeoXForCausalLM <-> CS 1.8 GPTJModel (configured as neox) with LM head"
    )
    def formats() -> Tuple[str, str]:
        return ("hf", "cs-1.8")
