...
class FinetuningTokenGenerator:
    def __init__(self, params, tokenizer, eos_id, pad_id):
        ...
        if self.is_multimodal:
            ...
            ###### for mllama
            self.image_ids = [self.image_token_id] * self.num_patches
        else:
            self.features = ["input_ids", "attention_mask", "labels"]
    ...
    def create_features_multimodal(
        ...
        labels = token_ids[1:]
        #### for mllama
        # replace the image token with pad token in the labels
        labels = [
            self.pad_id if x == self.image_token_id else x for x in labels
        ]
        input_mask = input_mask[1:]
    ...
    def tokenize_data(self, semantic_data_array):
        ...
        text_semantic_regions, image_regions = (
            data.get("text_semantic_regions"),
        ...
        for msg in conversation_data:
            if msg['role'] == 'user':
                # convert the content into a list containing dict
                msg_str = msg['content']
                # check if msg_str has image
                if self.image_token in msg_str:
                    msg['content'] = [
                        {'type': 'image'},
                        {
                            'type': 'text',
                            'text': msg_str.replace(self.image_token, ''),
                        },
                    ]
                else:
                    msg['content'] = [{'type': 'text', 'text': msg_str}]
        #####
            ...
            for id, offset, attention in zip(
                ...
                if id == self.image_token_id:
                    ...
                    loss_weight, attention_mask = (
                        image_regions[image_index].get("loss_weight"),
    ...
    def _encode(self, semantic_data_array):
        ...
        if not data:
            return {}, raw_data_stats
        tokenized_conversation_data, image_paths = (
            data.get("tokenized_data"),
            data.get("image_paths"),
        )
