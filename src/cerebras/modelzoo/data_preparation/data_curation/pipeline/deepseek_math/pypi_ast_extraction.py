...
#!/usr/bin/env python3
"""
Example usage of the AST Extraction Pipeline Step with datatrove.

This example shows how to set up and run a pipeline that processes
python repositories and extracts AST for all the python code files.
"""
import os
import sys
import time
...
from data_curation.pipeline.deepseek_math.extractors import ASTExtractor
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the AST extraction pipeline."
    )
    parser.add_argument(
        "--config",
        type=str,
...
def main():
    """Example of how to run the AST extraction pipeline."""
    # Parse arguments and load config once
    args = parse_args()
    config = load_config(args.config)
    # Get parameters from config with defaults
    start_subdir = config.get("pipeline", {}).get("start_subdir", 1)
    ...
    for subdir in range(start_subdir, end_subdir):
        ...
        # executor = LocalPipelineExecutor(pipeline=pipeline, **config["local_executor"])
        # Run the pipeline
        executor.run()
