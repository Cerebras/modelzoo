...
"""
Processor for the Stability dataset.
"""
import numpy as np
...
class StabilityDataProcessor(TfRecordsProcessor):
    """
    StabilityDataProcessor dataset processor for BERT regression fine-tuning task.
    ...
    def __init__(self, params):
        super(StabilityDataProcessor, self).__init__(
            params["data_dir"],
            params["batch_size"],
    ...
    def map_fn(self, raw_record):
        ...
        context_map = {
            ...
            "topology": tf.io.FixedLenFeature([], tf.string),
            "parent": tf.io.FixedLenFeature([], tf.string),
            "stability_score": tf.io.FixedLenFeature([1], tf.float32),
        }
        ...
        features = {
            ...
            "labels": tf.cast(
                tf.expand_dims(raw_context["stability_score"][0], -1),
                tf.float32,
            ),
        }
