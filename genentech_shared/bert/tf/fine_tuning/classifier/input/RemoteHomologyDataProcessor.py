...
"""
Processor for the RemoteHomology dataset.
"""
import numpy as np
...
class RemoteHomologyDataProcessor(TfRecordsProcessor):
    """
    RemoteHomologyDataProcessor dataset processor for BERT classification fine-tuning task.
    ...
    def __init__(self, params):
        super(RemoteHomologyDataProcessor, self).__init__(
            params["data_dir"],
            params["batch_size"],
    ...
    def map_fn(self, raw_record):
        ...
        of input features and labels for BERT classification fine-tuning task.
        """
        feature_map = {"primary": tf.io.FixedLenSequenceFeature([], tf.int64)}
        ...
        input_ids, input_mask = tf.numpy_function(
            ...
            [
                tf.cast(raw_features["primary"], tf.int32),
                tf.cast(raw_context["protein_length"], tf.int32),
            ],
            [tf.int32, tf.int32],
        )
        ...
        features = {
            ...
            "labels": tf.cast(raw_context["fold_label"], tf.int32),
        }
        label = tf.cast(raw_context["fold_label"], tf.int32)
