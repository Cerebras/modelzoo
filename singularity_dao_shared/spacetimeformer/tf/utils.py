...
from cerebras.pb.stack.autogen_pb2 import AP_WHITELISTED
...
def set_defaults(params):
    ...
    params["model"]["embedding_projection_initializer"] = params.get(
        "embedding_projection_initializer", {"name": "glorot_uniform"}
    )  # Used in the value and time embedding projection
    ...
    params["model"]["output_projection_initializer"] = params["model"].get(
        "output_projection_initializer", {"name": "glorot_uniform"}
    )
    ...
    params["train_input"]["src_vocab_size"] = 0
    params["train_input"]["tgt_vocab_size"] = 0
    ...
    if "eval_input" in params:
        params["eval_input"]["src_vocab_size"] = 0

        params["eval_input"]["tgt_vocab_size"] = 0
    else:
...
def get_custom_stack_params(params):
    ...
    if (
        ...
        config = FullConfig()
        stack_params["config"] = set_custom_config(config, params)

        # Use the new CIRH backend
